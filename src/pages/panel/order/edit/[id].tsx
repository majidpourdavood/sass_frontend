...
function UpdateDelivery(props) {
    ...
    const [initialValues, setInitialValues] = useState({
        client: props ? props.getOrder.data.clientId : "",
        delivery: props ? props.getOrder.data.deliveryId : "",
        origin: props ? props.getOrder.data.originId : "",
        destination: props ? props.getOrder.data.destinationId : "",
        amount_fuel: props ? props.getOrder.data.amountFuel : "",
    });
    ...
    const submitForm = async (e) => {
        ...
        let orderId = props.getOrder ? props.getOrder.data.id : "";
        let message = "";
        const url = process.env.URL_WEBSITE_SERVER + constants.URL_UPDATE_Order + orderId
        axios.patch(url, model,
            {
                headers: {
    ...
    return (
        <>
            <Head>
                <title>ManagePetro | Edit Order</title>
                <meta name="description" content="ManagePetro | Edit Order"/>
            </Head>
            <div className="container-fluid">
                <div className="card shadow mb-4">
                    <div className="card-header d-flex align-items-center py-3">
                        <h6 className="m-0 font-weight-bold text-primary col-6">Edit Order</h6>
                                    ...
                                    >
                                        ...
                                        {props.deliveries.data.map((delivery, key) => {
                                            return (
                                                <>
                                                    <option
                                                        selected={delivery.id == initialValues.delivery ? true : false}
                                    ...
                                    >
                                        ...
                                        {props.places.data.map((place, key) => {
                                            return (
                                                <>
                                                    <option
                                                        selected={place.id == initialValues.origin ? true : false}
                                    ...
                                    >
                                        ...
                                        {props.places.data.map((place, key) => {
                                            return (
                                                <>
                                                    <option
                                                        selected={place.id == initialValues.destination ? true : false}
                                                            value={place.id}>{place.name}</option>
                                            ...
                                            return (
                                                <>
                                                    <option value={client.id}
                                                            selected={client.id == initialValues.client ? true : false}
...
export async function getServerSideProps({req, res, params}) {
    ...
    try {
        ...
        const urlGetOrder = process.env.URL_WEBSITE_SERVER + constants.URL_GET_Order + idOrder;

        getOrder = await axios(urlGetOrder, {
            method: "GET",
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
